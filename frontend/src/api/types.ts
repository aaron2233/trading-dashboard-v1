...
export type RegimeLeveredConfluence =
  | "core_entry"
  | "overbought_watch"
  | "bull_no_trigger"
  | "own_regime_blocked"
...
export interface RegimeLeveredWeeklyState {
  bar_date: string | null;
  close: number | null;
  ma_10: number | null;
  ma_20: number | null;
  ...
  stoch_turned_up: boolean;
  close_above_20: boolean;
}

export interface RegimeLeveredSetup {
  ticker: string;
  bar_date: string | null;
  close: number | null;
  own_sqn_100: number | null;
  own_regime: string | null;
  weekly: RegimeLeveredWeeklyState | null;
  confluence: RegimeLeveredConfluence;
  rank_score: number;
  why_now: string;
  blockers: string[];
  // Unified scan-card fields
...
export interface RegimeLeveredScanRequest {
  tickers?: string[] | null;
  benchmark?: string;
}

export interface RegimeLeveredScanResponse {
  scan_time_utc: string;
  benchmark: string;
  broad_sqn_100: number | null;
  broad_regime: string | null;
  layer1_live: boolean;
  deployment_note: string;
  setups: RegimeLeveredSetup[];
  core_candidates: RegimeLeveredSetup[];
  dip_buy_signals: DipBuySignal[];
  errors: Record<string, string>;
}
