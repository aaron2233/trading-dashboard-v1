...
class TrancheResponse(BaseModel):
    id: int
    target_pct: float
    status: Literal["pending", "filled", "skipped"]
...
class PyramidResponse(BaseModel):
    ...
    horizon: str
    status: Literal["pending", "active", "completed", "stopped_out"]
    created_date: str
    closed_date: str | None = None
...
class FillTrancheRequest(BaseModel):
    tranche: Literal[1, 2, 3]
    ...
    expiry: str | None = None
    notes: str | None = None
    # Optional structure snapshot at fill time (T1 in particular) — used by
    # the T3 evaluator as the exact "new swing high" reference. When omitted
    # for T1, T3 falls back to the recent vs. prior swing comparison.
    swing_high_at_fill: float | None = None
    swing_low_at_fill: float | None = None
    # If true and structure capture fields aren't supplied, the API will pull
    # the current StructureRead via evaluate_pyramid and capture the
    # appropriate value automatically. Keeps the UI form simple.
    capture_structure: bool = False


class ClosePyramidRequest(BaseModel):
    completed: bool = False  # default to stopped_out
    pnl: float | None = None
    notes: str | None = None
...
class StructureRead(BaseModel):
    ...
    recent_swing_high_date: str | None = None
    recent_swing_low: float | None = None
    recent_swing_low_date: str | None = None
    prior_swing_high: float | None = None
    prior_swing_low: float | None = None
    pullback_held_20ma: bool = False
    pullback_held_50ma: bool = False
    rally_rejected_at_20ma: bool = False
    rally_rejected_at_50ma: bool = False
...
class TrancheTriggerResponse(BaseModel):
    tranche_id: int
    should_fire: bool
    blockers: list[str] = Field(default_factory=list)
    reasons: dict[str, str] = Field(default_factory=dict)
...
class ExitDirectiveResponse(BaseModel):
    action: str
    reason: str
    severity: Literal["info", "warn", "action"]
    affected_tranches: list[int] = Field(default_factory=list)
    # LEAPS roll/close economic context — None for non-LEAPS directives
    cost_basis_per_unit: float | None = None
    quantity: int | None = None
...
class PyramidEvaluationResponse(BaseModel):
    ticker: str
    direction: str
    bar_date: str
    close: float
    ...
    t1: TrancheTriggerResponse | None = None
    t2: TrancheTriggerResponse | None = None
    t3: TrancheTriggerResponse | None = None
    exits: list[ExitDirectiveResponse]
    next_tranche: int | None = None
