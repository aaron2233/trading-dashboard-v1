...
Production-personal. Tag `v0.1.0` ships when all three indicators match TradingView at >95% accuracy across 10 tickers — populate `tests/fixtures/truth/*.csv` with hand-sourced TradingView values (see `tests/fixtures/truth/README.md`); accuracy tests skip until fixtures exist, then fire automatically.
